subroutine danm_bnm(lat, nmax, cs, anm, bnm, lcanm, lcbnm)
!
! ==================================================================
!
! DESCRIPTION: This subroutine computes lumped coefficients for a single
!              latitude. The computation is performed in double precision.
!
!              The algorithm used compute Legendre functions in this subroutine
!              is numerically stable up to degree 1800 at most. Beyond this
...
! ==================================================================
    ...
    integer  :: n, n2, m
    real(dp) :: pnm0, pnm1, pnm2
    real(dp) :: u, um, t, sqrt3, prod, atmp, btmp

    real(dp), intent(out) :: lcanm(nmax + 1), lcbnm(nmax + 1)
    ! ===============================================
    ...
    do m = 0, nmax
        ...
        if (m == 0) then
            ...
            do n = 2, m
                n2 = 2 * n
                prod = prod * (real(n2, dp) + 1.0_dp) / real(n2, dp)
            end do
